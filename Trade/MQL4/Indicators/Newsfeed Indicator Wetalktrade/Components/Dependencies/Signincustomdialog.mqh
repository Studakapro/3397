...
//+------------------------------------------------------------------+
class CDialog3 : public CWndContainer
  {
private:
   //--- dependent controls
...
//| Constructor                                                      |
//+------------------------------------------------------------------+
CDialog3::CDialog3(void) : m_panel_flag(false),
                         m_minimized(false)
  {
  }
//+------------------------------------------------------------------+
//| Destructor                                                       |
//+------------------------------------------------------------------+
CDialog3::~CDialog3(void)
  {
  }
//+------------------------------------------------------------------+
//| Create a control                                                 |
//+------------------------------------------------------------------+
bool CDialog3::Create(const long chart,const string name,const int subwin,const int x1,const int y1,const int x2,const int y2)
  {
...
//+------------------------------------------------------------------+
bool CDialog3::Add(CWnd *control)
  {
...
//| Save                                                             |
//+------------------------------------------------------------------+
bool CDialog3::Save(const int file_handle)
  {
//--- check
...
//| Load                                                             |
//+------------------------------------------------------------------+
bool CDialog3::Load(const int file_handle)
  {
   if(file_handle==INVALID_HANDLE)
      return(false);
//--- load
...
//| Create "white border"                                            |
//+------------------------------------------------------------------+
bool CDialog3::CreateWhiteBorder(void)
  {
//--- coordinates
   int x1=0;
   int y1=0;
...
//--- create
   if(!m_white_border.Create(m_chart_id,m_name+"Border",m_subwin,x1,y1,x2,y2))
      return(false);
   if(!m_white_border.ColorBackground(clrWhite))
      return(false);
   if(!m_white_border.ColorBorder(clrWhite))
      return(false);
...
//| Create background                                                |
//+------------------------------------------------------------------+
bool CDialog3::CreateBackground(void)
  {
...
//--- create
   if(!m_background.Create(m_chart_id,m_name+"Back",m_subwin,x1,y1,x2,y2))
      return(false);
   if(!m_background.ColorBackground(clrWhite))
      return(false);
   color border=(m_panel_flag) ? clrWhite : clrWhite;
...
//| Create window title                                              |
//+------------------------------------------------------------------+
bool CDialog3::CreateCaption(void)
  {
...
//--- create
   if(!m_caption.Create(m_chart_id,m_name+"Caption",m_subwin,x1,y1,x2,y2))
      return(false);
   if(!m_caption.ColorBackground(clrWhite))
      return(false);
   if(!m_caption.ColorBorder(clrWhite))
      return(false);
...
//+------------------------------------------------------------------+
bool CDialog3::CreateButtonClose(void)
  {
   int off=(m_panel_flag) ? 0 : 2*CONTROLS_BORDER_WIDTH;
//--- coordinates
...
//| Create client area                                               |
//+------------------------------------------------------------------+
bool CDialog3::CreateClientArea(void)
  {
   int off=(m_panel_flag) ? 0:2*CONTROLS_BORDER_WIDTH;
//--- coordinates
...
//--- create
   if(!m_client_area.Create(m_chart_id,m_name+"Client",m_subwin,x1,y1,x2,y2))
      return(false);
   if(!m_client_area.ColorBackground(clrWhite))
      return(false);
   if(!m_client_area.ColorBorder(clrWhite))
      return(false);
...
//+------------------------------------------------------------------+
void CDialog3::OnClickCaption(void)
  {
  }
//+------------------------------------------------------------------+
//| Handler of click on the "Close" button                           |
//+------------------------------------------------------------------+
void CDialog3::OnClickButtonClose(void)
  {
   Visible(false);
  }
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//bool CDialog3::OnDialogDragStart(void)
//  {
...
////+------------------------------------------------------------------+
//bool CDialog3::OnDialogDragProcess(void)
//  {
////--- checking
//   if(m_drag_object==NULL)
//      return(false);
...
////+------------------------------------------------------------------+
//bool CDialog3::OnDialogDragEnd(void)
//  {
//   if(m_drag_object!=NULL)
//     {
...
//+------------------------------------------------------------------+
class CAppDialog3 : public CDialog3
  {
private:
   //--- dependent controls
...
//| Destructor                                                       |
//+------------------------------------------------------------------+
CAppDialog3::~CAppDialog3(void)
  {
  }
//+------------------------------------------------------------------+
//| Application dialog initialization function                       |
//+------------------------------------------------------------------+
bool CAppDialog3::Create(const long chart,const string name,const int subwin,const int x1,const int y1,const int x2,const int y2)
  {
   ...
   switch(m_program_type)
     {
      ...
      case PROGRAM_INDICATOR:
         ...
         break;
      default:
         Print("CAppDialog3: invalid program type");
         return(false);
     }
...
//+------------------------------------------------------------------+
bool CAppDialog3::CreateCommon(const long chart,const string name,const int subwin)
  {
//--- save parameters
   m_chart_id    =chart;
   m_name        =name;
...
//--- specify object and mouse events
   if(!m_chart.EventObjectCreate() || !m_chart.EventObjectDelete() || !m_chart.EventMouseMove())
     {
      Print("CAppDialog3: object events specify error");
      m_chart.Detach();
      return(false);
     }
...
//+------------------------------------------------------------------+
bool CAppDialog3::CreateExpert(const int x1,const int y1,const int x2,const int y2)
  {
...
//--- call method of the parent class
   if(!CDialog3::Create(m_chart.ChartId(),m_instance_id,m_subwin,x1,y1,x2,y2))
     {
      Print("CAppDialog3: expert dialog create error");
      m_chart.Detach();
      return(false);
     }
//--- succeed
...
//+------------------------------------------------------------------+
bool CAppDialog3::CreateIndicator(const int x1,const int y1,const int x2,const int y2)
  {
...
//--- determine subwindow
   ...
   if(m_subwin==-1)
     {
      Print("CAppDialog3: find subwindow error");
      m_chart.Detach();
      return(false);
     }
//---
...
//--- check if subwindow is not occupied by other indicators
   if(total!=1)
     {
      Print("CAppDialog3: subwindow busy");
...
//--- resize subwindow by dialog height
   if(!IndicatorSetInteger(INDICATOR_HEIGHT,(y2-y1)+1))
     {
      Print("CAppDialog3: subwindow resize error");
      ChartIndicatorDelete(m_chart.ChartId(),m_subwin,ChartIndicatorName(m_chart.ChartId(),m_subwin,total-1));
      m_chart.Detach();
      return(false);
     }
//--- indicator short name
   ...
   if(!IndicatorSetString(INDICATOR_SHORTNAME,m_indicator_name))
     {
      Print("CAppDialog3: shortname error");
      ChartIndicatorDelete(m_chart.ChartId(),m_subwin,ChartIndicatorName(m_chart.ChartId(),m_subwin,total-1));
      m_chart.Detach();
      return(false);
     }
//--- set flag 
   m_panel_flag=true;
//--- call method of the parent class
   if(!CDialog3::Create(m_chart.ChartId(),m_instance_id,m_subwin,0,0,width,y2-y1))
     {
      Print("CAppDialog3: indicator dialog create error");
      ChartIndicatorDelete(m_chart.ChartId(),m_subwin,ChartIndicatorName(m_chart.ChartId(),m_subwin,total-1));
      m_chart.Detach();
      return(false);
     }
...
//+------------------------------------------------------------------+
void CAppDialog3::Destroy(const int reason)
  {
//--- destroyed already?
...
//+------------------------------------------------------------------+
void CAppDialog3::SubwinOff(void)
  {
...
//+------------------------------------------------------------------+
bool CAppDialog3::CreateButtonMinMax(void)
  {
   int off=(m_panel_flag) ? 0:2*CONTROLS_BORDER_WIDTH;
//--- coordinates
...
//+------------------------------------------------------------------+
void CAppDialog3::ChartEvent(const int id,const long &lparam,const double &dparam,const string &sparam)
  {
...
//--- here we begin to assign IDs to controls
   if(Id(m_subwin*CONTROLS_MAXIMUM_ID)>CONTROLS_MAXIMUM_ID)
     {
      Print("CAppDialog3: too many objects");
      return(false);
     }
//--- succeed
   return(true);
...
//| Stop application                                                 |
//+------------------------------------------------------------------+
void CAppDialog3::OnClickButtonClose(void)
  {
//--- destroy application
   Destroy();
  }
...
//+------------------------------------------------------------------+
void CAppDialog3::OnClickButtonMinMax(void)
  {
...
//| Resize                                                           |
//+------------------------------------------------------------------+
bool CAppDialog3::Rebound(const CRect &rect)
  {
...
//+------------------------------------------------------------------+
void CAppDialog3::Minimize(void)
  {
//--- set flag
  // m_minimized=false;
//--- hide client area
...
//+------------------------------------------------------------------+
void CAppDialog3::Maximize(void)
  {
//--- reset flag
   m_minimized=false;
//--- show client area
...
//+------------------------------------------------------------------+
string CAppDialog3::CreateInstanceId(void)
  {
...
//+------------------------------------------------------------------+
void CAppDialog3::OnAnotherApplicationClose(const long &lparam,const double &dparam,const string &sparam)
  {
...
//+------------------------------------------------------------------+
void CAppDialog3::IniFileSave(void)
  {
...
//+------------------------------------------------------------------+
void CAppDialog3::IniFileLoad(void)
  {
   string filename=IniFileName()+IniFileExt();
...
//+------------------------------------------------------------------+
string CAppDialog3::IniFileName(void) const
  {
   string name;
//---
...
//| Load data                                                        |
//+------------------------------------------------------------------+
bool CAppDialog3::Load(const int file_handle)
  {
   if(CDialog3::Load(file_handle))
     {
      if(m_minimized)
        {
        // m_button_minmax.Pressed(true);
...
//| Save data                                                        |
//+------------------------------------------------------------------+
bool CAppDialog3::Save(const int file_handle)
  {
   if(CDialog3::Save(file_handle))
     {
