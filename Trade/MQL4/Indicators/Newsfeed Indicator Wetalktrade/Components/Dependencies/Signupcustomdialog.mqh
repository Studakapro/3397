...
//+------------------------------------------------------------------+
class CDialog2 : public CWndContainer
  {
private:
   //--- dependent controls
...
//| Constructor                                                      |
//+------------------------------------------------------------------+
CDialog2::CDialog2(void) : m_panel_flag(false),
                         m_minimized(false)
  {
  }
//+------------------------------------------------------------------+
...
//| Create a control                                                 |
//+------------------------------------------------------------------+
bool CDialog2::Create(const long chart,const string name,const int subwin,const int x1,const int y1,const int x2,const int y2)
  {
...
//+------------------------------------------------------------------+
bool CDialog2::Add(CWnd *control)
  {
...
//| Save                                                             |
//+------------------------------------------------------------------+
bool CDialog2::Save(const int file_handle)
  {
//--- check
...
//| Load                                                             |
//+------------------------------------------------------------------+
bool CDialog2::Load(const int file_handle)
  {
   if(file_handle==INVALID_HANDLE)
      return(false);
//--- load
...
//--- result
 ...
 color str_color2=StringToColor("66,165,246"); 
//+------------------------------------------------------------------+
//| Create "white border"                                            |
//+------------------------------------------------------------------+
bool CDialog2::CreateWhiteBorder(void)
  {
//--- coordinates
   int x1=0;
   int y1=0;
...
//--- create
   if(!m_white_border.Create(m_chart_id,m_name+"Border",m_subwin,x1,y1,x2,y2))
      return(false);
   if(!m_white_border.ColorBackground(str_color2))
      return(false);
   if(!m_white_border.ColorBorder(str_color2))
      return(false);
...
//| Create background                                                |
//+------------------------------------------------------------------+
bool CDialog2::CreateBackground(void)
  {
...
//--- create
   if(!m_background.Create(m_chart_id,m_name+"Back",m_subwin,x1,y1,x2,y2))
      return(false);
   if(!m_background.ColorBackground(str_color2))
      return(false);
   color border=(m_panel_flag) ? CONTROLS_DIALOG_COLOR_CLIENT_BORDER : CONTROLS_DIALOG_COLOR_CLIENT_BORDER;
...
//| Create window title                                              |
//+------------------------------------------------------------------+
bool CDialog2::CreateCaption(void)
  {
...
//--- coordinates
   ...
   int y1=off;
   int x2=Width()-off;
   int y2=y1+CONTROLS_DIALOG_CAPTION_HEIGHT+10;
//--- create
   if(!m_caption.Create(m_chart_id,m_name+"Caption",m_subwin,x1,y1,x2,y2))
      return(false);
   if(!m_caption.ColorBackground(str_color2))
      return(false);
   if(!m_caption.ColorBorder(str_color2))
      return(false);
   if(!m_caption.ReadOnly(true))
      return(false);
   if(!m_caption.Text(m_name))
      return(false);
    m_caption.Font("Calibri");
...
//+------------------------------------------------------------------+
bool CDialog2::CreateButtonClose(void)
  {
   int off=(m_panel_flag) ? 0 : 2*CONTROLS_BORDER_WIDTH;
//--- coordinates
...
//| Create client area                                               |
//+------------------------------------------------------------------+
bool CDialog2::CreateClientArea(void)
  {
   int off=(m_panel_flag) ? 0:2*CONTROLS_BORDER_WIDTH;
//--- coordinates
   int x1=off+CONTROLS_DIALOG_CLIENT_OFF;
   int y1=off+CONTROLS_DIALOG_CAPTION_HEIGHT+8;
...
//--- create
   if(!m_client_area.Create(m_chart_id,m_name+"Client",m_subwin,x1,y1,x2,y2))
      return(false);
   if(!m_client_area.ColorBackground(str_color2))
      return(false);
   if(!m_client_area.ColorBorder(str_color2))
      return(false);
...
//+------------------------------------------------------------------+
void CDialog2::OnClickCaption(void)
  {
  }
//+------------------------------------------------------------------+
//| Handler of click on the "Close" button                           |
//+------------------------------------------------------------------+
void CDialog2::OnClickButtonClose(void)
  {
   Visible(false);
  }
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//bool CDialog2::OnDialogDragStart(void)
//  {
...
////+------------------------------------------------------------------+
//bool CDialog2::OnDialogDragProcess(void)
//  {
////--- checking
//   if(m_drag_object==NULL)
//      return(false);
...
////+------------------------------------------------------------------+
//bool CDialog2::OnDialogDragEnd(void)
//  {
//   if(m_drag_object!=NULL)
//     {
...
//+------------------------------------------------------------------+
class CAppDialog2 : public CDialog2
  {
private:
   //--- dependent controls
...
//| Destructor                                                       |
//+------------------------------------------------------------------+
CAppDialog2::~CAppDialog2(void)
  {
  }
//+------------------------------------------------------------------+
//| Application dialog initialization function                       |
//+------------------------------------------------------------------+
bool CAppDialog2::Create(const long chart,const string name,const int subwin,const int x1,const int y1,const int x2,const int y2)
  {
   ...
   switch(m_program_type)
     {
      ...
      case PROGRAM_INDICATOR:
         ...
         break;
      default:
         Print("CAppDialog2: invalid program type");
         return(false);
     }
...
//+------------------------------------------------------------------+
bool CAppDialog2::CreateCommon(const long chart,const string name,const int subwin)
  {
//--- save parameters
   m_chart_id    =chart;
   m_name        =name;
...
//--- specify object and mouse events
   if(!m_chart.EventObjectCreate() || !m_chart.EventObjectDelete() || !m_chart.EventMouseMove())
     {
      Print("CAppDialog2: object events specify error");
      m_chart.Detach();
      return(false);
     }
...
//+------------------------------------------------------------------+
bool CAppDialog2::CreateExpert(const int x1,const int y1,const int x2,const int y2)
  {
...
//--- call method of the parent class
   if(!CDialog2::Create(m_chart.ChartId(),m_instance_id,m_subwin,x1,y1,x2,y2))
     {
      Print("CAppDialog2: expert dialog create error");
      m_chart.Detach();
      return(false);
     }
//--- succeed
...
//+------------------------------------------------------------------+
bool CAppDialog2::CreateIndicator(const int x1,const int y1,const int x2,const int y2)
  {
...
//--- determine subwindow
   ...
   if(m_subwin==-1)
     {
      Print("CAppDialog2: find subwindow error");
      m_chart.Detach();
      return(false);
     }
//---
...
//--- check if subwindow is not occupied by other indicators
   if(total!=1)
     {
      Print("CAppDialog2: subwindow busy");
...
//--- resize subwindow by dialog height
   if(!IndicatorSetInteger(INDICATOR_HEIGHT,(y2-y1)+1))
     {
      Print("CAppDialog2: subwindow resize error");
      ChartIndicatorDelete(m_chart.ChartId(),m_subwin,ChartIndicatorName(m_chart.ChartId(),m_subwin,total-1));
      m_chart.Detach();
      return(false);
     }
//--- indicator short name
   ...
   if(!IndicatorSetString(INDICATOR_SHORTNAME,m_indicator_name))
     {
      Print("CAppDialog2: shortname error");
      ChartIndicatorDelete(m_chart.ChartId(),m_subwin,ChartIndicatorName(m_chart.ChartId(),m_subwin,total-1));
      m_chart.Detach();
      return(false);
     }
//--- set flag 
   m_panel_flag=true;
//--- call method of the parent class
   if(!CDialog2::Create(m_chart.ChartId(),m_instance_id,m_subwin,0,0,width,y2-y1))
     {
      Print("CAppDialog2: indicator dialog create error");
      ChartIndicatorDelete(m_chart.ChartId(),m_subwin,ChartIndicatorName(m_chart.ChartId(),m_subwin,total-1));
      m_chart.Detach();
      return(false);
     }
...
//+------------------------------------------------------------------+
void CAppDialog2::Destroy(const int reason)
  {
//--- destroyed already?
...
//+------------------------------------------------------------------+
void CAppDialog2::SubwinOff(void)
  {
...
//+------------------------------------------------------------------+
bool CAppDialog2::CreateButtonMinMax(void)
  {
   int off=(m_panel_flag) ? 0:2*CONTROLS_BORDER_WIDTH;
//--- coordinates
...
//+------------------------------------------------------------------+
void CAppDialog2::ChartEvent(const int id,const long &lparam,const double &dparam,const string &sparam)
  {
...
//--- here we begin to assign IDs to controls
   if(Id(m_subwin*CONTROLS_MAXIMUM_ID)>CONTROLS_MAXIMUM_ID)
     {
      Print("CAppDialog2: too many objects");
      return(false);
     }
//--- succeed
   return(true);
...
//| Stop application                                                 |
//+------------------------------------------------------------------+
void CAppDialog2::OnClickButtonClose(void)
  {
//--- destroy application
   Destroy();
  }
...
//+------------------------------------------------------------------+
void CAppDialog2::OnClickButtonMinMax(void)
  {
...
//| Resize                                                           |
//+------------------------------------------------------------------+
bool CAppDialog2::Rebound(const CRect &rect)
  {
...
//+------------------------------------------------------------------+
void CAppDialog2::Minimize(void)
  {
//--- set flag
  // m_minimized=false;
//--- hide client area
...
//+------------------------------------------------------------------+
void CAppDialog2::Maximize(void)
  {
//--- reset flag
   m_minimized=false;
//--- show client area
...
//+------------------------------------------------------------------+
string CAppDialog2::CreateInstanceId(void)
  {
...
//+------------------------------------------------------------------+
void CAppDialog2::OnAnotherApplicationClose(const long &lparam,const double &dparam,const string &sparam)
  {
...
//+------------------------------------------------------------------+
void CAppDialog2::IniFileSave(void)
  {
...
//+------------------------------------------------------------------+
void CAppDialog2::IniFileLoad(void)
  {
   string filename=IniFileName()+IniFileExt();
...
//+------------------------------------------------------------------+
string CAppDialog2::IniFileName(void) const
  {
   string name;
//---
...
//| Load data                                                        |
//+------------------------------------------------------------------+
bool CAppDialog2::Load(const int file_handle)
  {
   if(CDialog2::Load(file_handle))
     {
      if(m_minimized)
        {
        // m_button_minmax.Pressed(true);
...
//| Save data                                                        |
//+------------------------------------------------------------------+
bool CAppDialog2::Save(const int file_handle)
  {
   if(CDialog2::Save(file_handle))
     {
