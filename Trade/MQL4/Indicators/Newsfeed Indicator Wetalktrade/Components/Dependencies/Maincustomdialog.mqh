...
//| Create "white border"                                            |
//+------------------------------------------------------------------+
bool CDialog1::CreateWhiteBorder(void)
  {
//--- coordinates
   int x1=0;
   int y1=0;
...
//--- create
   ...
   if(!m_white_border.ColorBackground(CONTROLS_DIALOG_COLOR_BG))
      return(false);
   if(!m_white_border.ColorBorder(str_color1))
      return(false);
...
//--- create
   ...
   if(!m_background.ColorBackground(clrWhite))
      return(false);
   color border=(m_panel_flag) ? str_color1 : str_color1;
...
//| Create window title                                              |
//+------------------------------------------------------------------+
 color str_color1=StringToColor("50,63,72");
...
//--- create
   if(!m_caption.Create(m_chart_id,m_name+"Caption",m_subwin,x1,y1,x2,y2))
      return(false);
   if(!m_caption.Color(clrWhite))
      return(false);
   if(!m_caption.ColorBackground(str_color1))
      return(false);
   if(!m_caption.ColorBorder(str_color1))
      return(false);
...
bool CDialog1::CreateButtonClose(void)
  {
   int off=(m_panel_flag) ? 0 : 2*CONTROLS_BORDER_WIDTH;
//--- coordinates
   int x1=Width()-off-(CONTROLS_BUTTON_SIZE+CONTROLS_DIALOG_BUTTON_OFF)-15;
   int y1=off+CONTROLS_DIALOG_BUTTON_OFF-5;
...
//--- coordinates
   ...
   int y1=off+CONTROLS_DIALOG_CAPTION_HEIGHT+10;
